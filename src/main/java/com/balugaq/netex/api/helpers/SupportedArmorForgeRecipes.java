...
@UtilityClass
public final class SupportedArmorForgeRecipes implements RecipeHolder, CanTestRecipe {
            ...
            if ((recipeType == RecipeType.ARMOR_FORGE) && allowedRecipe(item)) {
                ...
                for (ItemStack itemStack : item.getRecipe()) {
                    if (itemStack == null) {
                        ...
                        itemStacks[i] = ItemStackUtil.getCleanItem(itemStack);// new ItemStack(itemStack.clone());
                    }
                    if (++i >= 9) {
                        break;
                    }
