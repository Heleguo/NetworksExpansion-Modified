    ...
    private void handleFurnace(
        ...
        if (template.getType().isFuel()
            ...
            final ItemStack stack =
                    root.getItemStack(new ItemRequest(template, template.getMaxStackSize()));
            if (stack == null) {
                return;
            }
    ...
    private void handleBrewingStand(
        ...
        if (template.getType() == Material.BLAZE_POWDER) {
            if (brewer.getFuel() == null || brewer.getFuel().getType() == Material.AIR) {
                final ItemStack stack = root.getItemStack(
                        new ItemRequest(template.clone(), template.getMaxStackSize()));
                if (stack == null) {
                    return;
                }
